...
const BranchLookupModal = ({ isOpen, onClose, customParam }) => {
        ...
        fetchData("/lookupBranch", params)
            .then((result) => {
                if (result.success && result.data && result.data.length > 0 && result.data[0].result) {
                    const branchData = JSON.parse(result.data[0].result);
                    setBranches(branchData);
                    setFiltered(branchData); // Initialize filtered with all data
                } else {
                    console.warn(result.message || "No branches found.");
                    setBranches([]); // Ensure state is empty if no data
                    setFiltered([]);
                }
            })
            .catch((err) => {
    ...
    }, [isOpen, customParam]); // customParam added as dependency if its usage impacts fetch
