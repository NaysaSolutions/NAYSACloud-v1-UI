...
// /* -------- Timing (uses VITE_SESSION_LIFETIME minutes; default 60) -------- */
...
// const DYNAMIC_HEARTBEAT_MS = Math.max(IDLE_LIMIT_MS * 0.75, 30_000);

// /* ---------------- Leader heartbeat across tabs ---------------- */
// const AUTH_BC_NAME = "auth";
// const TAB_ID = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
// const HB_LEASE_KEY = "naysa_hb_leader";
// const HB_LEASE_MS = Math.max(DYNAMIC_HEARTBEAT_MS * 1.25, 45_000);
...
//     cacheUser(null);
//     markAuthReady(false);
//     // keep tenant unless you want to clear it: setTenant(null);
//   }, []);
...
//         } else {
//           // queue the popup for when tab becomes visible
...
//           setTenant(incoming, { silent: true }); // prevent echo loop
//         }
//       }
//     };
...
//   /* ---------------- On-focus/visible: show queued popup + light check ---------------- */
//   useEffect(() => {
//     let t = null;
//     const check = async () => {
//       try {
...
//   /* ---------------- Heartbeat + idle (leader-only heartbeat) ---------------- */
//   useEffect(() => {
//     if (!user) return;
//     let stopped = false;
...
//       if (leader && !stopped) {
//         const ok = await sendHeartbeat(); // on 401/419 -> broadcasts 'remote'
//         if (!ok) return;
//         renewLeader();
//       }
//       hbTimer.current = window.setTimeout(tick, DYNAMIC_HEARTBEAT_MS);
//     };
...
//       setTenant(companyCode); // broadcasts once if changed
