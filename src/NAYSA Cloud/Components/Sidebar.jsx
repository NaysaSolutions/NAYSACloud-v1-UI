...
const MenuItem = ({ item, level = 0, searchTerm, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (subMenuMatches) setIsOpen(true);
  ...
  const baseRow =
    "flex items-center justify-between py-2 px-3 rounded-xl transition-all duration-200 group hover:scale-[1.02]";
  const tone =
    level === 0
  ...
  // If this is a leaf (no submenu) -> render as a Link so it navigates
  if (!hasSubMenu) {
    return (
      <li>
        <Link
          to={path}
          className={`${baseRow} ${paddingLeft} ${tone}`}
          onClick={() => onNavigate?.()}
        >
          <div className="flex items-center space-x-3 flex-1 min-w-0">
            {level === 0 && Icon && (
              <Icon
                className={`text-xl flex-shrink-0 transition-colors duration-200 text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400`}
              />
            )}
            {level > 0 && (
              <div
  ...
  // With submenu -> clickable div that toggles open/close
  return (
    <li>
      <div
        className={`${baseRow} ${paddingLeft} ${tone} cursor-pointer`}
      ...
      subMenu: [
        {
          name: "Transactions",
          subMenu: [
            { name: "Accounts Payable Voucher", path: "/tran-ap-aptran" },  // <-- route to APV.jsx
        ...
        { name: "Inquiry", subMenu: ["AP Query", "VAT Input Query", "EWT Query", "2307 Monitoring", "Check Releasing and Return"] },
            ...
            <ul className="space-y-1">
              {menuItems.map((menuItem, index) => (
                <MenuItem
                  ...
                  item={menuItem}
                  searchTerm={searchTerm}
                  onNavigate={() => setIsSidebarOpen(false)} // close drawer on mobile after navigation
                />
              ))}
            </ul>
          </nav>
